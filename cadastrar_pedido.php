...
if (empty($num_pedido)) {
	...
	$ErrTexto = "<span style='font-size: 12px; color: #D90003;'>Número do Pedido é Obrigatorio!</span>";
		...
		$le_cotas = DBRead ('cotas', "WHERE (mes_cota = '$mes') AND (ano_cota = '$ano') AND (nome_loja = '$lojas')", '*');
		
		if ($le_cotas == false) {
				echo '<script> alert("Não Existe Cota para este mês ou para está loja!")</script>';
				echo "<script>location.href=('cadastrar_pedido.php')</script>";
				exit;
			}
			else {
				foreach ($le_cotas as $ct) {
					$idCota = $ct['id_cotas'];
					$CotaSub = $ct['cota_saldo'] - $valor;
					$GravaSaltoCota = array ('cota_saldo' => $CotaSub);
  					$AlteraSaldoCota = DBUpDate("cotas", $GravaSaltoCota, "id_cotas = '".$idCota."'");
				
					if ($ct['cota_saldo'] <= 0 ) {
					echo '<script> alert("Saldo da '.$ct['nome_loja'].' de '.$ct['mes_cota'].' de ' .$ct['ano_cota']. ' está Negativa!")</script>';
					}
				}
			}
...
<table border="0" cellpadding="0" cellspacing="0" class="table">
  <tbody>
    ...
    <tr>
      ...
      <td width="79%">
      	<input type="text" name="num_pedido" class="cod" onkeypress='validate(event)' <?php echo $Errborda; ?>>
    ...
    <tr>
      ...
      <input type="text" name="qtd" class="cod" onkeypress='validate(event)'>
      </td>
    </tr>
    <tr>
      <td align="right">Data de Entrega:</td>
      <td><input type="date" name="data_entrega" class="cod" id="data" maxlength="10" placeholder="dd/mm/aaaa" onkeyup="formatar('##/##/####', this, event)" onkeypress='validate(event)'></td>
    </tr>
    <tr>
  ...
  <td>
       <input type="text" id="categoria" name="condicoes" class="cod" placeholder="Código" onkeypress='validate(event)'><br /> 
    ...
    <tr>
      ...
      <td width="79%"><input type="text" name="descricao" class="text" style="width: 512px;" onkeypress='validateChar(event)'></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
