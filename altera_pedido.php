...
if (empty($num_pedido)) {
	...
	$ErrTexto = "<span style='font-size: 12px; color: #D90003;'>Número do Pedido é Obrigatorio!</span>";
		...
		$mes = date('m', strtotime($entrega));
		$ano = date('Y', strtotime($entrega));
		...
		switch ($mes) {
			...
			$pedido = array (
			  ...
			  if ($altera = DBUpDate('pedidos', $pedido, 'id_pedidos="'.$id_pedido.'"')) {
				  	if(!empty($arrayParcelas)) {
						  $contador = 1;
              $exclui_datas_vencimento = DBExcluir('datas_pag_pedidos', 'cod_pedido="'.$num_pedido.'"');
                ...
                'cod_pedido' => $num_pedido,
                  ...
                  $grava_datas_vencimento = DBCreate('datas_pag_pedidos', $datas, 'cod_pedido="'.$num_pedido.'"');
			  ...
			  if ($altera) {
				  echo "<script> alert('Pedido Nº $num_pedido alterado com sucesso! $cod_pedido') </script>";
				  echo "<script>location.href=('visualiza_pedido.php')</script>";
			  }
			  else { 
				  echo "Erro ao alterar pedido nº $num_pedido, tente novamente mais tarde!";
				  exit;
			  }
      }
}
...
<table border="0" cellpadding="0" cellspacing="0" class="table">
  <tbody>
    <tr>
        <td height="42" colspan="2" align="center"><span class="titulo">Alterar Pedido Nº <?php echo $pd['num_pedido'];?></span></td>
    </tr>
    <tr>
      <td width="21%" align="right">Num. do Pedido: <input type="hidden" name="id_pd" value="<?php echo $pd['id_pedidos']; ?>"></td>
      <td width="79%">
      	<input type="text" name="num_pedido" class="cod" value="<?php echo $pd['num_pedido']; ?>">
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
      <input name="valor" class="cod" onkeyup="soma()" onKeyPress="return(MascaraMoeda(this,'.',',',event))" value="<?php echo number_format($pd['valor_pedido'], 2, ',', '.'); ?>"/>
      <span style="margin-left: 20px;">Quantidade: </span>
      <input type="text" name="qtd" class="cod" onkeypress='validate(event)' value="<?php echo $pd['qtd_pedido']; ?>">
      </td>
    </tr>
    <tr>
      <td align="right">Data de Entrega:</td>
      <td><input type="date" name="data_entrega" class="cod" id="data" maxlength="10" placeholder="dd/mm/aaaa" onkeyup="formatar('##/##/####', this, event)" onkeypress='validate(event)' value="<?php echo $entrega; ?>"></td>
    </tr>
    <tr>
...
<?php
  ...
  $contador = 0;  
    foreach ($datas_pag as $dt) {
      ...
      $tipo_pagamento = "
          ...
          if ($exibeVencimeto==1){ 
            $data_vencimentos = $datas_pagamento;
          }
  ...
  <td>
       <input type=\"text\" id=\"categoria\" name=\"condicoes\" class=\"cod\" placeholder=\"Código\" onkeypress='validate(event)' value=".$pd['cod_cond_pag']."><br /> 
       <span style=\"font-size:10px; color:#7B7B7B; margin-bottom: 10px;\">*Aperte TAB para atualizar os valores!</span> <br />
       <input name=\"prazo\" class=\"cod\" id=\"parcela\" onkeyup=\"soma()\" onKeyDown=\"validar(this,'text');\" value='".$pd['prazo']."' style=\"width: 8px; float: left; text-align: right; border:0; background-color:transparent;\">
  		  <span style=\"padding-top:5px; float:left;\">x</span>
        <span style=\"margin-left: 20px;\">De: R$ </span>
        <input type=\"text\" name=\"valor_dividido\" class=\"cod\" onKeyPress=\"return(MascaraMoeda(this,'.',',',event))\" readonly value='".number_format($pd['valor_dividido'], 2, ',', '.')."' style=\"border:0; background-color:transparent;\" > <br />
       <div id=\"subCategoria\" class=\"data_parcela\"> ".$data_vencimentos."</div>
    </td>
    </tr>
      ";
    }
    ...
    <tr>
      ...
      <td width="79%"><input type="text" name="descricao" class="text" style="width: 512px;" onkeypress='validateChar(event)'  value="<?php echo $pd['descricao']; ?>"></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
